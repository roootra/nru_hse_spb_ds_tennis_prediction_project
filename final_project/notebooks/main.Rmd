---
title: "Tennis prediction project"
...
```{r}
atp <- read_csv("~/private/final_project/origin_data/ATP/ATP.csv", 
...
library(partykit)
library(caret)
test_ctree <- ctree(win ~ ., data = atp_data_avg)
plot(test_ctree)
test_ctree_predict <- predict(test_ctree, atp_data_avg)
confusionMatrix(test_ctree_predict, atp_data_avg$win)
...
```{r}
wta <- read_csv("~/private/final_project/origin_data/WTA/WTA.csv", 
    col_types = cols(WRank = col_double(), LRank = col_double(),
                     WPts = col_double(), LPts = col_double(),
        AvgL = col_double(), 
...
library(partykit)
library(caret)
test_ctree <- ctree(win ~ ., data = wta_data_avg)
plot(test_ctree)
test_ctree_predict <- predict(test_ctree, wta_data_avg)
confusionMatrix(test_ctree_predict, wta_data_avg$win)
